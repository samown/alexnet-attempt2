"""
AlexNet Modified 1: With Batch Normalization
Adds Batch Normalization after each convolutional and fully connected layer
"""
...
class AlexNetModified1(nn.Module):
    """
    AlexNet with Batch Normalization
    
    Modifications:
    - Batch Normalization after each convolutional layer
    - Batch Normalization after FC layers (except output)
    - Maintains ReLU activation and dropout
    """
    
    def __init__(self, num_classes: int = 251, dropout: float = 0.5):
        """
        Initialize AlexNet Modified 1 (with Batch Normalization)
        
        Args:
            num_classes: Number of output classes (default: 251 for iFood 2019)
            dropout: Dropout probability for FC layers (default: 0.5)
        """
        super(AlexNetModified1, self).__init__()
        
        self.num_classes = num_classes
        self.dropout = dropout
        
        # Feature extraction (Convolutional layers with Batch Normalization)
        ...
        # Classifier (Fully Connected layers with Batch Normalization)
